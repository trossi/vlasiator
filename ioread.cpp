...
template<unsigned long int N> bool readFsGridVariable(
   ...
   if(decomposition == fileDecomposition) {
      ...
      for(FsGridTools::Task_t task = 0; task < myRank; task++) {
         ...
         thatTasksSize[0] =
             FsGridTools::calcLocalSize(globalSize[0], decomposition[0], task / decomposition[2] / decomposition[1]);
         thatTasksSize[1] =
             FsGridTools::calcLocalSize(globalSize[1], decomposition[1], (task / decomposition[2]) % decomposition[1]);
      ...
      for(int task = 0; task < numWritingRanks; task++) {
         ...
         thatTasksSize[0] = FsGridTools::calcLocalSize(globalSize[0], fileDecomposition[0],
                                                       task / fileDecomposition[2] / fileDecomposition[1]);
         thatTasksSize[1] = FsGridTools::calcLocalSize(globalSize[1], fileDecomposition[1],
                                                       (task / fileDecomposition[2]) % fileDecomposition[1]);
         thatTasksSize[2] =
             FsGridTools::calcLocalSize(globalSize[2], fileDecomposition[2], task % fileDecomposition[2]);

         thatTasksStart[0] = FsGridTools::calcLocalStart(globalSize[0], fileDecomposition[0],
                                                         task / fileDecomposition[2] / fileDecomposition[1]);
         thatTasksStart[1] = FsGridTools::calcLocalStart(globalSize[1], fileDecomposition[1],
                                                         (task / fileDecomposition[2]) % fileDecomposition[1]);
         thatTasksStart[2] =
             FsGridTools::calcLocalStart(globalSize[2], fileDecomposition[2], task % fileDecomposition[2]);
